...
export default function BlueskyLayout() {
   ...
   const { isLoggedIn, user, loading: authLoading, error: authError, refresh: refreshAuth } = useAuth();
   ...
   const filteredFeed = useMemo(() => {
      ...
      const items = annonces.filter((item) => {
         const queryMatch = normalizedQuery.length === 0 || item.title?.toLowerCase().includes(normalizedQuery) || String(item.description || "").toLowerCase().includes(normalizedQuery);
         if (!queryMatch) return false;
         if (showOnlyReposts && item.feedType !== "repost") return false;
         return true;
      });
      if (sortMode === "engaged") {
         return [...items].sort((left, right) => (Number(right.commentCount || 0) + Number(right.repostCount || 0)) - (Number(left.commentCount || 0) + Number(left.repostCount || 0)));
      }
      return items;
   ...
   const handleCloseModal = () => { setIsModalOpen(false); setSelectedPost(undefined); };

   const handleRepost = (postId: string) => {
      const annonceToRepost = annonces.find(a => a.id === postId);
      if (annonceToRepost) { setSelectedPost(annonceToRepost); setRepost(true); }
   };
         ...
         <div className="sticky top-[4.35rem] md:top-16 bg-white/90 border border-gray-100 rounded-2xl z-10 backdrop-blur shadow-sm p-1">
            <div ref={tabsContainerRef} className="relative">
               <div className="flex bg-gray-50 rounded-xl p-1">
                  <button ref={btnJobRef} className={`flex-1 py-2.5 md:py-3 text-center text-xs md:text-sm font-semibold rounded-xl transition-colors duration-150 ${tabs === "job-seeker" ? "text-orange-600" : "text-gray-600 hover:bg-gray-100"}`} onClick={() => setTabs("job-seeker")} aria-pressed={tabs === "job-seeker"} aria-label="Afficher les annonces">Announce</button>
                  <button ref={btnEmpRef} className={`flex-1 py-2.5 md:py-3 text-center text-xs md:text-sm font-semibold rounded-xl transition-colors duration-150 ${tabs === "employer" ? "text-orange-600" : "text-gray-600 hover:bg-gray-100"}`} onClick={() => setTabs("employer")} aria-pressed={tabs === "employer"} aria-label="Afficher le fil">feeds</button>
               </div>
               <span aria-hidden className="absolute bottom-0 h-0.5 bg-orange-500 rounded-full transition-all duration-300 ease-out" style={{ left: indicator.left, width: indicator.width }} />
            </div>
            ...
            {loading ? (
               ...
               <div className="p-4">
                  <div className="mx-auto max-w-3xl rounded-lg border border-red-100 bg-red-50 p-4 text-center text-red-800">
                     ...
                     <button onClick={() => fetchAnnonces()} className="inline-flex items-center rounded-md bg-red-600 px-3 py-1.5 text-sm font-semibold text-white hover:bg-red-700">Réessayer</button>
                  </div>
               </div>
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white pt-20">
         <div className="fixed top-0 z-20 w-full border-b border-gray-200 bg-white/90 p-3 backdrop-blur sm:p-4">
            <div className="mx-auto flex max-w-7xl items-center justify-between">
               ...
               <button type="button" aria-label="Ouvrir les filtres" className="inline-flex items-center gap-1 rounded-xl border border-orange-200 bg-orange-50 px-3 py-1.5 text-xs font-semibold text-orange-700 xl:hidden focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2" onClick={() => setIsFilterOpen(true)}>
         ...
         <button onClick={handleOpenModal} aria-label="Créer une annonce" className="fixed lg:hidden bottom-5 right-4 z-30 bg-orange-500 text-white rounded-2xl px-4 py-3 shadow-lg hover:bg-orange-600 transition-colors flex items-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2">
            <Plus className="w-5 h-5" />
            <span className="text-sm font-semibold">Announces</span>
         </button>

         <FilterDrawer isFilterOpen={isFilterOpen} setIsFilterOpen={setIsFilterOpen} searchQuery={searchQuery} setSearchQuery={setSearchQuery} showOnlyReposts={showOnlyReposts} setShowOnlyReposts={setShowOnlyReposts} sortMode={sortMode} setSortMode={setSortMode} />

         <PostModal isOpen={isModalOpen} onClose={handleCloseModal} onSuccess={fetchAnnonces} post={selectedPost} />
         ...
         {selectedPost && repost && (
            <Repost date={selectedPost.created_at || new Date()} postId={String(selectedPost.id)} alreadyReposted={false} post={selectedPost} isOpen={repost} onClose={() => setRepost(false)} onSuccess={fetchAnnonces} />
         )}

         {deleteTargetId !== null && (
            <DeleteModal isOpen={isDeleteConfirmVisible} onClose={closeDeleteConfirmModal} onConfirm={confirmDeletePost} isDeletingPost={isDeletingPost} />
         )}
