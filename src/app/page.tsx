    ...
    try {
      ...
      const response = await fetch("/api/announcements", {
        ...
        body: JSON.stringify({
          ...
          // Ajoutez d'autres champs nécessaires selon votre modèle IAnnouncement
        }),
      });
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {announcements.map((post) => (
          <div
            key={post.id}
            className="bg-white p-4 rounded-lg shadow-md flex flex-col min-h-48"
          >
...
const DynamicListAnnouncement = ({
  ...
  const renderContent = () => {
    ...
    while (index < announcements.length) {
      if (rows.length % 4 === 0) {
        // Ligne avec 3 annonces (1 grande + 2 petites)
        rows.push(
        ...
        // Ligne avec 3 annonces (2 petites + 1 grande)
        rows.push(
          <div key={rows.length} className="grid grid-cols-3 gap-4 mb-4">
