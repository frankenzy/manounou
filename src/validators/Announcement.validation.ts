...
export class AnnouncementValidator {
  static validateCreate(announcementData: Omit<IAnnouncement, 'id' | 'createdAt' | 'updatedAt'>): ValidationResult {
    const errors: string[] = [];
    ...
    if (!announcementData.description || announcementData.description.trim().length === 0) {
  ...
  static validateUpdate(announcementData: Partial<IAnnouncement>): ValidationResult {
    const errors: string[] = [];
